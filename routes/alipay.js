/**
 * Created by webxiaoz@163.com on 2019/4/8
 */
...
let alipay_gate_way = 'https://openapi.alipay.com/gateway.do';
let alipay_gate_way_sandbox = 'https://openapi.alipaydev.com/gateway.do';
...
props.pay = function (opts) {
    ...
    let params = this.makeParams('alipay.trade.app.pay', biz_content);
...
props.webPay = function (opts) {
    ...
    let params = this.makeParams('alipay.trade.page.pay', biz_content);
    params.notify_url = this.notifyUrl;
    console.log(params,'web')
    return utl.processParams(params, this.rsaPrivate, this.signType);
};

/**
 * 创建二维码订单
 * @param {Object} opts 支付参数
 ...
 * @return {Promise}
 */
props.createQRPay = function (opts) {
        let biz_content = {
            body: opts.body,
            subject: opts.subject,
            out_trade_no: opts.outTradeId,
        ...
        let params = this.makeParams("alipay.trade.precreate", biz_content);
        params.notify_url = this.notifyUrl;
...
props.signVerify = function (response) {
    let ret = utl.copy(response);
    let sign = ret['sign'];
    ret.sign = undefined;
    ret.sign_type = undefined;

    let tmp = utl.encodeParams(ret);
...
props.query = function (opts) {
    ...
    let body = utl.processParams(params, this.rsaPrivate, this.signType);
...
props.close = function (opts) {
    ...
    let params = this.makeParams('alipay.trade.close', biz_content);
    if(this.appAuthToken) {
        params.app_auth_token = this.appAuthToken;
    }
...
props.refund  = function (opts) {
    ...
    let params = this.makeParams('alipay.trade.refund', biz_content);
    if(this.appAuthToken) {
        params.app_auth_token = this.appAuthToken;
    }
...
props.refundQuery = function (opts) {
    ...
    let params = this.makeParams('alipay.trade.fastpay.refund.query', biz_content);
    if(this.appAuthToken) {
        params.app_auth_token = this.appAuthToken;
    }
...
props.billDownloadUrlQuery = function (opts) {
    ...
    let params = this.makeParams('alipay.data.dataservice.bill.downloadurl.query', biz_content);
    if(this.appAuthToken) {
        params.app_auth_token = this.appAuthToken;
    }
